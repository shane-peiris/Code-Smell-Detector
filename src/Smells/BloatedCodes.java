...
public class BloatedCodes {
    ...
    public DefaultTableModel getLongMethods(SeperateFileContent SFC)
    {
    ...
    public DefaultTableModel getLargeClasses(SeperateFileContent SFC )
    {
        ...
        for(int c=0;c<SFC.ClassCodeBlocks.size();c++)
        {
            ...
            {
                model.addRow(new Object[] {ClassName.toString(),String.valueOf(MethodCount),String.valueOf(AttributeCount),String.valueOf(LineCount),String.valueOf(StartLineNo)});
            }
            ...
            if(((MethodDefinition)SFC.MethodCodeBlocks.elementAt(m)).para_Defs.size()>=MAX_PARAMETER_COUNT)
            {
                ...
                for(int c=0;c<SFC.ClassCodeBlocks.size();c++)
                {
                    for(int x=0;x<((ClassDefinition)SFC.ClassCodeBlocks.elementAt(c)).method_names.size();x++)
                    {
                        ...
                        {
                            ClassContain = ((ClassDefinition)SFC.ClassCodeBlocks.elementAt(c)).class_name.toString();
                            model.addRow(new Object[] {MethName.toString(),ClassContain.toString(),String.valueOf(ParameterCount),String.valueOf(StartLineNo)});
                        }
                    }
                }
            }
